...
class BinConverter():
    ...
    def dictionary():
        ...
        """
        for key, value in BinConverter.letter_dict.items():
            print('{} -> {}'.format(key, value))

    def encrypt(message):
        """This converts an alpha-numeric string into a binary output based on the binary dictionary in the script
        """
        bin_temp = [BinConverter.letter_dict[each_letter] for each_letter in message]
    ...
    def decrypt(message):
        ...
        for _ in range(1, char_count + 1):
            temp.append(BinConverter.binary_dict[message[((current_count * binary_length) - binary_length):(current_count * binary_length)]])
            current_count += 1
        return ''.join(temp)
    ...
    def convert():
        ...
        if confirm == '1' or confirm == 'text' or confirm == 't' or confirm[0] == 't':
            ...
            print(BinConverter.encrypt(message))
            ...
            print(BinConverter.decrypt(message))
        else:
